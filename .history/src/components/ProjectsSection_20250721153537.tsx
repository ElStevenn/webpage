...
const PROJECTS: Project[] = [
  {
    ...
    blurb: 'Streamline incoming mail, trigger smart workflows & ensure zero‑loss delivery.',
    icon: MailOpen,
  ...
  {
    title: 'Plan Your Inheritance',
    blurb: 'Estate‑planning SaaS that crunches tax codes & generates the paperwork.',
    icon: ShieldCheck,
    highlight: true
  },
  {
  ...
  {
    title: 'Nexus Cloud',
    blurb: 'Autoscaling edge platform with real‑time metrics & cost guardrails.',
    icon: Cloud
  }
] as const
...
export default function ProjectsSection() {
  ...
  return (
    <section id="projects" className="relative py-32 md:py-48 overflow-hidden">
      {/* top wave divider – sits above hero fade */}
      <motion.svg
      ...
      >
        <path
          d="M0 100 Q 360 0 720 60 T 1440 40 V100 H0 Z"
          fill="url(#waveFill)"
        />
        <defs>
      ...
      <div ref={pinRef} className="container mx-auto px-6 max-w-7xl">
        ...
        <ul className="mt-24 grid gap-12 sm:grid-cols-2 lg:grid-cols-3">
          {PROJECTS.map(({ icon: Icon, title, blurb, link, highlight }, i) => (
            <motion.li
              key={title}
