// Services – premium consultation packs with dynamic badges and richer backdrop
'use client'
...
interface Pack {
  ...
  comingSoon?: boolean    // greys‑out & disables booking
}
  ...
  return (
    <section id="services" className="relative py-32 md:py-48 overflow-hidden">
      ...
      <motion.svg
        ...
        className="absolute -top-px left-0 right-0 w-full -z-10 select-none"
        viewBox="0 0 1440 100" preserveAspectRatio="none"
      >
        <path d="M0 100 Q 360 0 720 60 T 1440 40 V100 H0 Z" fill="url(#waveFillS)" />
        <defs>
      ...
      {/* subtle grain */}
      <div className="absolute inset-0 -z-10 pointer-events-none mix-blend-overlay opacity-5 bg-[url('/grain.png')] bg-repeat" />
      {/* animated blobs */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 0.13, x: [0, -70, 60, 0], y: [0, 80, -60, 0] }}
        transition={{ repeat: Infinity, duration: 46, ease: 'easeInOut' }}
        className="absolute right-0 top-1/3 w-[60vw] h-[60vw] rounded-full blur-3xl bg-[conic-gradient(#ff5e5b_15%,#58a6ff_45%,#ffc371_75%,#ff5e5b_100%)] -z-20" />
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 0.1, x: [0, 60, -50, 0], y: [0, -70, 50, 0] }}
        transition={{ repeat: Infinity, duration: 52, ease: 'easeInOut', delay: 4 }}
        className="absolute -left-32 bottom-1/4 w-[55vw] h-[55vw] rounded-full blur-3xl bg-[conic-gradient(#58a6ff_20%,#ffc371_50%,#ff5e5b_80%,#58a6ff_100%)] -z-20" />
        ...
        <ul className="mt-24 grid gap-12 sm:grid-cols-2 lg:grid-cols-3">
          {PACKS.map(({ icon: Icon, title, blurb, price, tag, comingSoon }, i) => (
            <motion.li
              ...
              className={`relative group rounded-3xl p-8 backdrop-blur-lg border border-white/10 shadow-xl transition-transform ${comingSoon ? 'bg-white/5 opacity-50 cursor-default' : 'bg-white/5 hover:shadow-2xl'}`}
            >
              {/* badge */}
              {tag && (
                <span className={`absolute -top-3 -right-3 inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold ring-1 ring-white/20 backdrop-blur-md shadow-inner ${comingSoon ? 'bg-white/15 text-white/70' : 'bg-brand-blue/30 text-white'}`}>
                  {tag}
                </span>
              )}

              <Icon className="w-12 h-12 text-brand-blue drop-shadow-[0_0_8px_#58a6ff90] ${comingSoon ? 'grayscale' : 'group-hover:scale-110'} transition-transform" />
              ...
              <p className="mt-4 text-lg font-semibold bg-gradient-to-r from-brand-blue via-sky-300 to-brand-blue bg-clip-text text-transparent">
                {price}
              </p>
              ...
              {comingSoon ? (
                <button
                  disabled
                  className="inline-block mt-6 px-5 py-2 rounded-md text-sm font-semibold text-white/40 bg-white/10 cursor-not-allowed"
                >
