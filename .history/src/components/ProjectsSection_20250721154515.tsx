...
const PROJECTS: Project[] = [
  ...
  },
  {
    ...
    icon: Receipt,
    tag: 'Private Project'    // new subtle badge
  }
] as const
        ...
        <ul className="mt-24 grid gap-12 sm:grid-cols-2 lg:grid-cols-3">
          {PROJECTS.map(({ icon: Icon, title, blurb, link, highlight, tag }, i) => (
            ...
            >
              ...
              {!highlight && tag && (
                <span className="absolute -top-3 -right-3 px-3 py-1 rounded-full bg-white/15 text-xs font-semibold text-white/70 border border-white/20 backdrop-blur-sm">
                  {tag}
                </span>
              )}
